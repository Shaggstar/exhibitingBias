...
// Minimal mythic glyphs used in backdrop (JSX only, no TS types)
const mythGlyphs = [
  (props) => (
...
// Generic section + primitives (no TS generics)
...
const Chip = ({ children }) => (
  <span className="inline-flex items-center gap-2 text-xs tracking-wide uppercase rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[color:var(--subtext)]">
    {children}
  </span>
);

const Card = ({ children, glow, onClick }) => (
  <motion.div
    onClick={onClick}
    whileHover={{ y: -3 }}
    className={`relative rounded-2xl p-5 md:p-6 bg-[color:var(--surface)] border border-white/8 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] ${
      glow ? "ring-1 ring-[color:var(--accent)]/40" : ""
    }`}
  >
    {children}
  </motion.div>
...
const AlignmentPill = ({ label, desc }) => (
  <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 bg-white/6 border border-white/10 rounded-xl px-4 py-3">
...
const FRONTIERS_MOH_URL =
  "https://www.frontiersin.org/journals/sociology/articles/10.3389/fsoc.2023.1269621/full";
const TMS_URL =
  "https://www.dropbox.com/scl/fi/wokkcyflsm1yojotsyhey/TmS__Frontiers.pdf?rlkey=3d6my2ggke9bpcna98429bb0x&dl=0";
...
const POEMS = [
  ...
  { kind: "Note", title: "1st Draft", note: "misc notes", substackUrl: "" },
...
export default function MythWireframe() {
  ...
  return (
    ...
    >
      ...
      <nav className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-black/30 bg-black/20 border-b border-white/10">
        <div className="max-w-6xl mx-auto px-4 md:px-8 h-14 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-5 text-[color:var(--subtext)]">
            <a href="#narrative" className="hover:text-[color:var(--text)]">
              Narrative
            </a>
            <a href="#alignment" className="hover:text-[color:var(--text)]">
              Alignment
            </a>
            <a href="#poetry" className="hover:text-[color:var(--text)]">
              Poetry & Notes
            </a>
            <a href="#projects" className="hover:text-[color:var(--text)]">
              Projects
            </a>
            <a href="#research" className="hover:text-[color:var(--text)]">
              Research
            </a>
            <a href="#sim" className="hover:text-[color:var(--text)]">
              Sim Lab
            </a>
            <a href="#about" className="hover:text-[color:var(--text)]">
              About
            </a>
          </div>
          <button
            onClick={() => setDark(!dark)}
            className="inline-flex items-center gap-2 text-sm text-[color:var(--subtext)] hover:text-[color:var(--text)]"
          >
          ...
          <div className="grid md:grid-cols-2 gap-10 items-start">
            <div>
              <div className="flex items-center gap-2 mb-4">
                <Chip>New Human Narrative</Chip>
              </div>
              <h1 className="font-serif text-3xl md:text-5xl tracking-tight text-[color:var(--text)]">
                From <span className="text-[color:var(--accent)]">Morals</span> → Symbols → Culture
              </h1>
              <p className="text-[color:var(--subtext)] mt-2 italic">
                “… to ensure that the robots build better humans …”
              ...
              <p className="text-[color:var(--subtext)] mt-4 max-w-xl">
                Add the preface: <b>Accent Bias</b> → Morals → Symbols → Culture. Formalize it (TMS), write it
                (vignettes/poems/essays), then test it (simulations) to steer governance.
              </p>
              <div className="flex flex-wrap items-center gap-4 mt-6">
                <a
                  href="#poetry"
                  className="inline-flex items-center gap-2 rounded-full bg-[color:var(--accent)] text-black px-5 py-2.5 text-sm font-medium"
                >
                  Read drafts <ArrowRight className="h-4 w-4" />
                </a>
                <a
                  href="#sim"
                  className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-5 py-2.5 text-sm text-[color:var(--text)]"
                >
                  Open Sim Lab
                </a>
              </div>
            ...
            <Card glow>
              ...
              <ul className="text-sm text-[color:var(--subtext)] space-y-2 list-disc pl-5">
                ...
                <li>
                  Every section framed by <em>story → formalism → simulation</em>
                </li>
              </ul>
            </Card>
          </div>
      ...
      <Section
        id="narrative"
        title="The Narrative: Origins & Nature"
        subtitle="A living manuscript—told in vignettes, poems, and essays—paired with formal notes and runnable sketches."
        icon={ScrollText}
      >
        <div className="grid md:grid-cols-2 gap-4">
          <Card>
            ...
            <p className="text-sm text-[color:var(--subtext)]">
              Perceptual accent biases seed local preferences; aggregated norms stabilize symbols; symbols scale identity and
              governance.
            </p>
          </Card>
          <Card>
            ...
            <p className="text-sm text-[color:var(--subtext)]">
              Treat moral priors as control parameters that regulate populations of agents—predicting cohesion, sanction,
              forgiveness.
            </p>
          </Card>
        </div>
        <div className="grid md:grid-cols-2 gap-4">
          <Card>
            ...
            <ul className="text-sm text-[color:var(--subtext)] space-y-2">
              {PROSE.map((p) => (
                <li key={p.url}>
                  <a className="underline hover:text-[color:var(--text)]" href={p.url} target="_blank" rel="noreferrer">
                    {p.title}
                  </a>
                </li>
              ))}
          ...
          <Card>
            ...
            <div className="text-sm text-[color:var(--subtext)] space-y-2">
              <div className="flex items-center gap-2">
                ...
                <a className="underline hover:text-[color:var(--text)]" href={FRONTIERS_MOH_URL} target="_blank" rel="noreferrer">
                  MOH (Frontiers in Sociology)
                </a>
              </div>
              <div className="flex items-center gap-2">
                <FileText className="h-4 w-4 text-[color:var(--accent)]" />
                <a className="underline hover:text-[color:var(--text)]" href={TMS_URL} target="_blank" rel="noreferrer">
                  Transcendental Model Selection (manuscript)
                </a>
              </div>
            </div>
          </Card>
      ...
      <Section
        ...
        subtitle="Past, present, and future tech woven into a single thread—individual flourishing to AGI governance."
        icon={Compass}
      >
        <div className="grid md:grid-cols-3 gap-4">
          <Card glow>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="font-medium text-[color:var(--text)]">Future Tech</h4>
            </div>
            <p className="text-sm text-[color:var(--subtext)]">
              Strengthen governance systems to align AGI with human values: normative priors, oversight councils, simulation
              audits.
            </p>
          </Card>
          <Card>
            <div className="flex items-center gap-3 mb-2">
              <PenTool className="h-5 w-5 text-[color:var(--accent)]" />
              <h4 className="font-medium text-[color:var(--text)]">Current Tech</h4>
            </div>
            <p className="text-sm text-[color:var(--subtext)]">
              Tools that optimize our broader Selves: personal narrative tooling, Better Self agents, normative recommender
              prototypes.
            </p>
          </Card>
          <Card>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="font-medium text-[color:var(--text)]">Past Tech</h4>
            </div>
            <p className="text-sm text-[color:var(--subtext)]">
              Traditions of wisdom that scaffold identity scaling: myths, epics, ritual—rendered as computable motifs.
            </p>
          </Card>
        </div>
        <div className="grid gap-3">
          <AlignmentPill
            label="Accent Bias → Morals → Symbols → Culture"
            desc="Biases in perception seed moral norms; norms stabilize symbols; symbols scale into cultural governance."
          />
          <AlignmentPill
            label="Story → Formalism → Simulation"
            desc="Each idea is narrated, expressed in MOH/TMS mathematics, then evaluated in agent-based sims."
          />
          <AlignmentPill
            label="Identity Scaling"
            desc="Morality as precision that lets agents inhabit larger groups—basis for symbolic cognition and governance."
          />
        </div>
      </Section>
      ...
      <Section
        id="poetry"
        title="Poetry & Notes (Raw Drafts)"
        subtitle="Framed by ‘Finding Our Place in the Age of Robot Gods’: free-form expression first, refinement later. Use AI as a tutor—not a substitute."
        icon={Feather}
      >
        <div className="flex items-center justify-between">
            ...
            {["All", "Draft", "Note", "Embed"].map((t) => (
              <button
                ...
                className={`px-4 py-1.5 text-sm rounded-full transition ${
                  filter === t ? "bg-[color:var(--accent)] text-black" : "text-[color:var(--subtext)] hover:text-[color:var(--text)]"
                }`}
              >
                {t}
              </button>
          ...
          <a
            className="text-sm text-[color:var(--subtext)] hover:text-[color:var(--text)] inline-flex items-center gap-2"
            href="https://substack.com/@shaggy"
            target="_blank"
            rel="noreferrer"
          >
            All on Substack <ExternalLink className="h-4 w-4" />
          </a>
        </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="mt-3 flex gap-3 text-sm">
                    {substackUrl ? (
                      <a className="px-3 py-1.5 rounded-full bg-[color:var(--accent)] text-black" href={substackUrl} target="_blank" rel="noreferrer">
                        Open on Substack
                      </a>
                    ) : (
                      <button className="px-3 py-1.5 rounded-full border border-white/10 bg-white/5 text-[color:var(--subtext)] cursor-not-allowed">
                        Substack
                      </button>
                    )}
                  </div>
      ...
      <Section id="projects" title="Active Projects" subtitle="Translating theory into practice." icon={FlaskConical}>
        <div className="grid md:grid-cols-2 gap-4">
          <Card glow>
            ...
            <p className="text-sm text-[color:var(--subtext)]">
              Normative-aware agent that co-authors goals with you; balances personal priors with communal norms.
            </p>
          </Card>
          <Card glow>
            <div className="flex items-center gap-3 mb-2">
              <Network className="h-5 w-5 text-[color:var(--accent)]" />
              <h4 className="font-medium text-[color:var(--text)]">Discovery Engine</h4>
            </div>
            <p className="text-sm text-[color:var(--subtext)]">
              AI-driven synthesis of scientific fields into computable knowledge—supports hypothesis generation and governance audits.
            </p>
          </Card>
        </div>
      </Section>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Card>
            ...
            <p className="text-sm text-[color:var(--subtext)] mb-3">
              Peer‑reviewed article outlining the Myth of Objectivity and its cultural implications.
            </p>
            <a className="inline-flex items-center gap-2 text-sm underline" href={FRONTIERS_MOH_URL} target="_blank" rel="noreferrer">
          ...
          <Card>
            ...
            <p className="text-sm text-[color:var(--subtext)] mb-3">
              Draft manuscript connecting MOH to a computational governance formalism.
            </p>
            <a className="inline-flex items-center gap-2 text-sm underline" href={TMS_URL} target="_blank" rel="noreferrer">
              <ExternalLink className="h-4 w-4" />
              Open manuscript
            </a>
          </Card>
      ...
      <Section
        ...
        subtitle="Sandbox scenarios for MOH/TMS agents. Explore how narrative templates and moral precision regulate culture."
        icon={Wand2}
      >
        <div className="grid md:grid-cols-3 gap-4">
          <Card>
      ...
      <Section
        ...
        subtitle="Awarded an Active Inference Institute fellowship to formalize MOH/TMS and build simulations."
        icon={BookOpen}
      >
        <div className="grid md:grid-cols-3 gap-4">
          <Card>
            ...
            <p className="text-sm text-[color:var(--subtext)]">
              Myth of Objectivity & Origins of Symbols → TMS → Simulated governance.
            </p>
          </Card>
          <Card>
            <h4 className="font-medium text-[color:var(--text)] mb-2">Writing</h4>
            <p className="text-sm text-[color:var(--subtext)]">
              Vignettes, poems, and essays that make the formal intuitions tangible.
            </p>
          </Card>
          <Card>
            <h4 className="font-medium text-[color:var(--text)] mb-2">Contact</h4>
            <p className="text-sm text-[color:var(--subtext)]">
              Invite talks, collaborations, and pilots. Newsletter signup fits here.
            </p>
          </Card>
        </div>
      </Section>
      ...
      <footer className="border-t border-white/10 py-10">
        <div className="max-w-6xl mx-auto px-4 md:px-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="text-xs text-[color:var(--subtext)]">
            © {new Date().getFullYear()} • Red/Orange minimal theme • Built with care.
          </div>
        </div>
      </footer>
    </div>
