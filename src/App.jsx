...
import {
  BookOpen, Brain, Compass, ScrollText, FlaskConical, Network,
  Wand2, Sparkles, Rocket, PenTool, ArrowRight, ExternalLink,
  Sun, Moon, Feather, Landmark, FileText
} from "lucide-react";

import MarkdownModal from "./components/MarkdownModal";
import { getMarkdownIndex, filterBySection } from "./lib/useMarkdownIndex";
...
// Minimal mythic glyphs used in backdrop (pure JS — no TS types)
const mythGlyphs = [
  (props) => (
    ...
    <div className="absolute inset-0">
      {mythGlyphs.map((G, i)=> (
        <motion.div
          ...
          style={{ left: `${15 + i*22}%`, top: `${(i%3)*25 + 10}%`, color: palette.orangeSoft }}
        >
...
const Card = ({children, glow, onClick}) => (
  <motion.div
    onClick={onClick}
    whileHover={{ y: -3 }}
    className={`relative rounded-2xl p-5 md:p-6 bg-[color:var(--surface)] border border-white/8 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] ${glow ? "ring-1 ring-[color:var(--accent)]/40" : ""}`}
  >
    {children}
  </motion.div>
);
...
export default function MythWireframe(){
  ...
  const openMarkdown = (item) => setOpenItem(item);
  const closeMarkdown = () => setOpenItem(null);
  ...
  return (
    ...
    >
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="hidden md:inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[color:var(--accent)] text-black hover:opacity-90 text-sm"
            >
              ☕ Ko‑fi
            </a>
            <button
            ...
            <div>
              ...
              <p className="text-[color:var(--subtext)] mt-2 italic">
                ...
                <span className="opacity-70"> —{" "}
                  <a className="underline" href="https://shaggy.substack.com/p/finding-our-place-in-the-age-of-robot" target="_blank" rel="noreferrer">
                    Finding Our Place in the Age of Robot Gods
                  </a>
                </span>
              </p>
              <p className="text-[color:var(--subtext)] mt-4 max-w-xl">
                Add the preface: <b>Accent Bias</b> → Morals → Symbols → Culture. Formalize it (TMS), write it (vignettes/poems/essays),
                then test it (simulations) to steer governance.
              </p>
              <div className="flex flex-wrap items-center gap-4 mt-6">
                <a href="#poetry" className="inline-flex items-center gap-2 rounded-full bg-[color:var(--accent)] text-black px-5 py-2.5 text-sm font-medium">
                  Read drafts <ArrowRight className="h-4 w-4"/>
                </a>
                <a href="#sim" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-5 py-2.5 text-sm text-[color:var(--text)]">
                  Open Sim Lab
                </a>
              </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Card>
            ...
            <p className="text-sm text-[color:var(--subtext)]">
              Perceptual accent biases seed local preferences; aggregated norms stabilize symbols; symbols scale identity and governance.
            </p>
          </Card>
          <Card>
            <h4 className="font-medium text-[color:var(--text)] mb-2">Regulation by Morality</h4>
            <p className="text-sm text-[color:var(--subtext)]">
              Treat moral priors as control parameters that regulate populations of agents—predicting cohesion, sanction, forgiveness.
            </p>
          </Card>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <Card glow>
            <div className="flex items-center gap-3 mb-2">
              <Rocket className="h-5 w-5 text-[color:var(--accent)]"/><h4 className="font-medium text-[color:var(--text)]">Future Tech</h4>
            </div>
            <p className="text-sm text-[color:var(--subtext)]">
              Strengthen governance systems to align AGI with human values: normative priors, oversight councils, simulation audits.
            </p>
          </Card>
          <Card>
            <div className="flex items-center gap-3 mb-2">
              <PenTool className="h-5 w-5 text-[color:var(--accent)]"/><h4 className="font-medium text-[color:var(--text)]">Current Tech</h4>
            </div>
            <p className="text-sm text-[color:var(--subtext)]">
              Tools that optimize our broader Selves: personal narrative tooling, Better Self agents, normative recommender prototypes.
            </p>
          </Card>
          <Card>
            <div className="flex items-center gap-3 mb-2">
              <ScrollText className="h-5 w-5 text-[color:var(--accent)]"/><h4 className="font-medium text-[color:var(--text)]">Past Tech</h4>
            </div>
            <p className="text-sm text-[color:var(--subtext)]">
      ...
      >
        <div className="flex items-center justify-between">
          <div className="inline-flex rounded-full bg-white/5 p-1 border border-white/10">
            {['All','Draft','Note','Embed'].map(t=> (
              <button
                key={t}
                onClick={()=>setFilter(t)}
                className={`px-4 py-1.5 text-sm rounded-full transition ${filter===t?"bg-[color:var(--accent)] text-black":"text-[color:var(--subtext)] hover:text-[color:var(--text)]"}`}
              >
                {t}
              </button>
            ))}
          ...
          <a
            ...
            href="https://substack.com/@shaggy" target="_blank" rel="noreferrer"
          >
            All on Substack <ExternalLink className="h-4 w-4"/>
          </a>
        </div>

        {/* Cards sourced from POEMS list */}
        ...
        {/* Local Markdown items (from /public/content) */}
        {(poetryItems.length > 0 || essayItems.length > 0) && (
          <Card>
            <h4 className="font-medium text-[color:var(--text)] mb-2">Local drafts (Markdown)</h4>
            <div className="grid md:grid-cols-2 gap-3">
              <div>
                <p className="text-xs uppercase tracking-wide text-[color:var(--subtext)] mb-2">Poems</p>
                <ul className="space-y-2">
                  {poetryItems.map(item => (
                    <li key={item.path}>
                      <button
                        onClick={()=>openMarkdown(item)}
                        className="text-sm underline hover:text-[color:var(--text)] text-[color:var(--subtext)]"
                      >
                        {item.title}
                      </button>
                    </li>
              ...
              <div>
                <p className="text-xs uppercase tracking-wide text-[color:var(--subtext)] mb-2">Essays</p>
                <ul className="space-y-2">
                  {essayItems.map(item => (
                    <li key={item.path}>
                      <button
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Card>
            ...
            <a className="inline-flex items-center gap-2 text-sm underline" href={FRONTIERS_MOH_URL} target="_blank" rel="noreferrer">
              <ExternalLink className="h-4 w-4"/>Read on Frontiers
            </a>
          </Card>
          <Card>
            ...
            {TMS_URL ? (
              <a className="inline-flex items-center gap-2 text-sm underline" href={TMS_URL} target="_blank" rel="noreferrer">
                <ExternalLink className="h-4 w-4"/>Open manuscript
              </a>
            ) : (
      ...
      {/* Markdown modal */}
      {openItem && (
        <MarkdownModal item={openItem} onClose={closeMarkdown}/>
      )}
    </div>
  );
}
