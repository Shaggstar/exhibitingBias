...
import { ProseForPathway } from "./prose";
...
const LINKS: Link[] = [
  ...
  { source: "null_hypothesis", target: "homo_economicus" },
  { source: "null_hypothesis", target: "single_equilibrium" },
  { source: "null_hypothesis", target: "binary_opposition" },
...
function ConceptualAtlas() {
  ...
  useEffect(() => {
    if (selectedPathway === 0 && showProse) {
      setShowProse(false);
    }
  }, [selectedPathway, showProse]);
              ...
              return (
                ...
                >
                  <span className="inline-flex items-center gap-2">
                    ...
                    <span>{p === 0 ? `All (${count})` : `P${p} (${count})`}</span>
                  </span>
                </button>
              );
            })}
          ...
          <div className="space-y-2 max-h-[48vh] overflow-auto pr-1">
            {selectedPathway === 0 ? (
              <div className="text-xs text-[color:var(--accentSoft)] px-1">
                Select a pathway tab to browse its nodes, or click directly in the graph.
              </div>
            ) : (
              <>
                <div className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--accentSoft)] px-1">
                  Pathway {selectedPathway}: {PATHWAY_LABEL[selectedPathway] ?? ""}
                </div>
                {visibleNodes.map((it) => (
                  <button
                    key={it.id}
              ...
              return (
                <div>
                  ...
                  <div className="mt-4 pt-4 border-t border-white/10">
                    <button
                      type="button"
                      onClick={() => setShowProse((prev) => !prev)}
                      disabled={selectedPathway === 0}
                      className={`px-3 py-1.5 text-xs rounded-lg border border-white/10 transition ${
                        selectedPathway === 0
                          ? "bg-black/20 text-[color:var(--accentSoft)] cursor-not-allowed"
                          : showProse
                          ? "bg-[color:var(--accent)]/20 text-[color:var(--text)]"
                          : "bg-white/5 hover:bg-white/10"
                      }`}
                    >
                      {showProse ? "Hide Prose" : "Show Prose"}
                    </button>

                    {showProse && (
                      <div className="mt-3 space-y-3 text-sm leading-relaxed text-[color:var(--text)]/90">
                        {selectedPathway === 0 ? (
                          <p className="text-xs text-[color:var(--accentSoft)]">
                            Choose a specific pathway tab to view its prose.
                          </p>
                        ) : (
                          <ProseForPathway pathway={selectedPathway} />
                        )}
                      </div>
                    )}
                  </div>
